  ...
  async createUser({ request, response }) {
    ...
    if(checkUsernameEmail && checkUsernameEmail.email === data.email) 
      return response.status(403).send(
        { error: 
          { message: 'Email já existe', name: 'Forbidden', status: 403 } 
        }
      )
    if(checkUsernameEmail && checkUsernameEmail.username === data.username) 
      return response.status(403).send(
        { error: 
          { message: 'Username já existe', name: 'Forbidden', status: 403 }
        }
      )
    ...
    if(!createUser) 
      return response.status(500).send(
        { error: 
          { message: `Algo de errado aconteceu`, name: 'Internal Server Error', status: 500 }
        }
      )
